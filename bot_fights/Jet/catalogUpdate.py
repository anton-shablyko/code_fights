def catalogUpdate(catalog, updates):
    d = {}
    for cat in catalog:  # conver catalog to dict
        d[cat[0]] = cat[1:]

    for cat in updates:  # add items to dict
        if cat[0] in d.keys():
            d[cat[0]] += cat[1:]
        else:
            d[cat[0]] = cat[1:]

    final_catalog = []  # convert dict to list
...
"""
Jet.com customers can easily find the item they are looking for by browsing by category.
Categories are stored in a catalog that is updated on a regular basis as inventory changes.
Your goal is to implement an algorithm that updates the catalog with new items.
